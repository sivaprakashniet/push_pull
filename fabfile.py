"""
Starter fabfile for deploying the push_and_pull project.
...
# CHANGEME
env.hosts = ['user@push_and_pull.example.com']
env.code_dir = '/srv/www/push_and_pull'
env.project_dir = '/srv/www/push_and_pull/push_and_pull'
env.static_root = '/srv/www/push_and_pull/static/'
env.virtualenv = '/srv/www/push_and_pull/.virtualenv'
env.code_repo = 'git@github.com:user/{{project_name}}.git'
env.django_settings_module = 'push_and_pull.settings'
...
def restart():
    ...
    with cd(env.code_dir):
        run("touch %s/push_and_pull/wsgi.py" % env.code_dir)
